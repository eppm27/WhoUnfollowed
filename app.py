...
@app.route("/debug")
def debug():
    """Debug route to help troubleshoot path issues."""
    return {
    ...
    try:
        ...
        if "following" not in request.files or "followers" not in request.files:
            return (
                jsonify(
                    {
                        "error": "Both following.json and followers.json files are required"
                    }
                ),
                400,
            )
...
def download_csv_data():
    try:
        ...
        return send_file(
            csv_file, as_attachment=True, download_name=filename, mimetype="text/csv"
        )
