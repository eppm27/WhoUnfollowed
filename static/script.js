...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function checkAnalyzeButton() {
        const followingValid = followingFile.files.length > 0 && validateJSONFile(followingFile.files[0]).valid;
        const followersValid = followersFile.files.length > 0 && validateJSONFile(followersFile.files[0]).valid;
    ...
    function displayUserList(elementId, users) {
        ...
        if (users.length === 0) {
            listElement.innerHTML = '<div class="user-item" style="text-align: center; color: #8e8e8e; font-style: italic;">No users found! 🎉</div>';
            return;
        }
